...
final _expandedLibraryProvider = FutureProvider.autoDispose<_ExpandedLibraryData>((ref) async {
  ...
  return _ExpandedLibraryData(folders: folders, subjects: subjects, notes: notes);
});
...
class LibraryExpandedTree extends ConsumerWidget {
  ...
  List<Widget> _childrenForLocation(_ExpandedLibraryData data) => switch (location) {
    LibraryRootLocation() => [
      ...data.rootFolders.map((folder) => _FolderNode(data: data, folder: folder)),
      if (data.ungroupedBooks.isNotEmpty)
        ExpansionTile(
          leading: const Icon(Icons.inbox_outlined),
          title: const Text('未归类书籍'),
          children: data.ungroupedBooks.map((book) => _SubjectNode(data: data, subject: book, depth: 0)).toList(),
        ),
      ...data.rootNotes.map((note) => _NoteTile(note: note, depth: 0)),
    ],
    LibraryFolderLocation(:final folderId) => [
      if (data.folderById(folderId) case final folder?) _FolderNode(data: data, folder: folder),
    ],
    LibraryUngroupedBooksLocation() => [
      ...data.ungroupedBooks.map((book) => _SubjectNode(data: data, subject: book, depth: 0)),
    ],
    LibrarySubjectLocation(:final subjectId) => [
      if (data.subjectById(subjectId) case final subject?) _SubjectNode(data: data, subject: subject, depth: 0),
    ],
  };
}
...
class _ExpandedLibraryData {
  ...
  const _ExpandedLibraryData({required this.folders, required this.subjects, required this.notes});

  List<SubjectFolder> get rootFolders => folders.where((folder) => folder.parentId == null).toList();
  SubjectFolder? folderById(String id) => folders.where((folder) => folder.id == id).firstOrNull;
  Subject? subjectById(String id) => subjects.where((subject) => subject.id == id).firstOrNull;
  List<SubjectFolder> foldersIn(String id) => folders.where((folder) => folder.parentId == id).toList();
  List<Subject> booksIn(String? folderId) => subjects.where((subject) => subject.level == 0 && subject.folderId == folderId).toList();
  List<Subject> childrenOf(String id) => subjects.where((subject) => subject.parentId == id).toList();
  List<Note> notesIn(String id) => notes.where((note) => note.subjectId == id).toList();
  List<Note> get rootNotes => notes.where((note) => note.subjectId == defaultSubjectId).toList();
...
class _FolderNode extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ExpansionTile(
    ...
    children: [
      ...data.foldersIn(folder.id).map((child) => _FolderNode(data: data, folder: child)),
      ...data.booksIn(folder.id).map((book) => _SubjectNode(data: data, subject: book, depth: 0)),
    ],
  );
}
...
class _SubjectNode extends StatelessWidget {
  ...
  final int depth;
  const _SubjectNode({required this.data, required this.subject, required this.depth});

  @override
  Widget build(BuildContext context) {
    final children = data.childrenOf(subject.id);
    final notes = data.notesIn(subject.id);
    final icon = const {0: Icons.menu_book_outlined, 1: Icons.bookmark_outline, 2: Icons.article_outlined}[subject.level]!;
    ...
    return ExpansionTile(
      ...
      children: [
        ...
        ...children.map((child) => _SubjectNode(data: data, subject: child, depth: depth + 1)),
      ],
    );
  }
}
